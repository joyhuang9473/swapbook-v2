...
forge script script/1_DeploySwapbookAVS.s.sol \
...
if [ $? -eq 0 ]; then
    ...
    echo "🎉 SwapbookAVS has been deployed to Base Testnet"
    echo "📋 Check the output above for the contract address"
    echo ""
    echo "🔧 Next steps:"
    echo "1. Set the attestation center: swapbookAVS.setAttestationCenter(address)"
    echo "2. Set the SwapbookV2 hook: swapbookAVS.setSwapbookV2(address)"
    echo "3. Users can now deposit funds for limit orders"
else
