...
/**
 * @title Scenario2_LimitOrderAndSwap
 * @notice Scenario where SWAPBOOK_USER places a limit order and UNISWAP_USER executes a large swap
 * @dev This scenario demonstrates:
 * 1. SWAPBOOK_USER places a limit order (tick=60, inputAmount=100 tokenA, outputAmount=100.6)
 * 2. UNISWAP_USER executes a large swap through Universal Router that moves pool tick > 60
 * 3. SwapbookV2 hook is triggered during the swap (afterSwap)
...
contract Scenario2_LimitOrderAndSwap is Script {
    ...
    function run() public {
        console.log("=== Scenario 2: Limit Order and Large Swap ===");
        console.log("SWAPBOOK_USER places limit order (tick=60), UNISWAP_USER executes large swap");
        console.log("This will trigger SwapbookV2 hook afterSwap and process the limit order");
        ...
        // Step 4: UNISWAP_USER executes large swap through Universal Router
        console.log("\n=== Step 4: UNISWAP_USER Executes Large Swap ===");
        _executeLargeSwap(universalRouter, poolManager, swapbookV2, poolId, key, token0, token1, uniswapUserAddress, swapbookUserAddress, swapbookAVS);
        
        // Step 4.5: Analyze price impact
        console.log("\n=== Step 4.5: Price Impact Analysis ===");
        _analyzePriceImpact(poolManager, poolId);
        ...
        console.log("\n=== Scenario 2 Complete ===");
        console.log("[SUCCESS] Limit order was placed and large swap was executed successfully!");
        console.log("[SUCCESS] SwapbookV2 hook was triggered and processed the limit order!");
        console.log("[SUCCESS] UNISWAP_USER's large swap moved the pool tick above 60!");
    ...
    function _checkInitialState(
        ...
        uint256 pendingOrderBefore = swapbookV2.pendingOrders(poolId, 60, false);
    ...
    function _placeLimitOrder(
        ...
        // tick=60, inputAmount=100 tokenA, outputAmount=100.6 tokenB
        bytes memory updateTaskData = abi.encode(
            ...
            4000e18,        // inputAmount (4000 tokenA)
            4007e18,        // outputAmount (4007 tokenB)
        ...
        console.log("  Tick: 60");
        console.log("  Input Amount: 4000 tokenA (to be sold)");
        console.log("  Output Amount: 4007 tokenB (expected to receive)");
        console.log("  Escrow: 4000 tokenA (deposited for order)");
        console.log("  User:", swapbookUserAddress);
        
        // Check swapbook user's token0 balance (needed for the limit order)
        ...
        if (swapbookUserBalance < 4000e18) {
            console.log("ERROR: Swapbook user doesn't have enough token0");
            console.log("Required: 4000e18 (4000 tokens)");
        ...
        // For a limit order at tick 60 (selling token0 for token1), we need token0 in escrow
        // because the order will sell token0 when filled
        console.log("Approving SwapbookAVS to spend 4000 tokenA from swapbook user...");
        vm.startBroadcast(swapbookUserPrivateKey);
        MockERC20(token0Address).approve(address(swapbookAVS), 4000e18);
        vm.stopBroadcast();
        
        console.log("Depositing 4000 tokenA from swapbook user to SwapbookAVS escrow...");
        vm.startBroadcast(swapbookUserPrivateKey);
        swapbookAVS.depositFunds(token0Address, 4000e18);
        vm.stopBroadcast();
    ...
    function _checkStateAfterLimitOrder(
        ...
        // Check SwapbookV2 state
        uint256 pendingOrderAfter = swapbookV2.pendingOrders(poolId, 60, false);
    ...
    function _executeLargeSwap(
        ...
        console.log("Executing large swap through Universal Router...");
        console.log("UNISWAP_USER wants to buy tokenA with large amount of tokenB");
        console.log("This should move the pool tick above 60 to trigger afterSwap hook");
        ...
        // Calculate expected execution and price impact
        (uint160 sqrtPriceX96, int24 currentTick, , ) = poolManager.getSlot0(poolId);
        ...
        // Use a large amount to ensure we move the tick above 60
        uint256 largeAmountIn = 4500e18; // 4,500 tokenB
        console.log("Large swap amount:", largeAmountIn, "tokenB");
        
        // Create swap parameters for Universal Router
        // This will trigger the SwapbookV2 hook during the swap
        console.log("Preparing large swap parameters:");
        console.log("  Pool Manager:", address(poolManager));
        console.log("  Pool Key:", uint256(PoolId.unwrap(poolId)));
        console.log("  Zero for One: false (buying token0 with token1)");
        console.log("  Amount Specified:", largeAmountIn, "token1");
        ...
        token1.approve(permit2, largeAmountIn);
        console.log("Approved Permit2 to spend", largeAmountIn, "token1");
        permit2Contract.approve(address(token1), address(universalRouter), uint160(largeAmountIn), uint48(block.timestamp + 600));
        console.log("Uses Permit2 to approve the UniversalRouter with a specific amount and expiration time.");
        vm.stopBroadcast();
        
        // Expect the LimitOrderExecutedAfterSwap event to be emitted during the swap
        vm.expectEmit(true, true, true, true);
        emit SwapbookV2.LimitOrderExecutedAfterSwap();
        ...
        IV4Router.ExactInputSingleParams memory swapParams = IV4Router.ExactInputSingleParams({
            ...
            amountIn: uint128(largeAmountIn), // exact input amount
        ...
        params[1] = abi.encode(key.currency1, largeAmountIn);
        ...
        console.log("[SUCCESS] LimitOrderExecutedAfterSwap event was emitted!");
        ...
        // Check if pool tick moved above 60
        (uint160 newSqrtPriceX96, int24 newTick, , ) = poolManager.getSlot0(poolId);
        console.log("New pool tick after swap:", newTick);
        console.log("New pool sqrtPriceX96:", newSqrtPriceX96);
        
        if (newTick > 60) {
            console.log("[SUCCESS] Pool tick moved above 60! Tick:", newTick);
            console.log("[SUCCESS] This triggered the afterSwap hook and processed the limit order!");
        } else {
            console.log("[INFO] Pool tick did not move above 60. Current tick:", newTick);
        }
    }
    
    function _analyzePriceImpact(
        IPoolManager poolManager,
        PoolId poolId
    ) internal view {
        console.log("Analyzing price impact from large swap...");
        ...
        if (currentTick > 60) {
            console.log("[SUCCESS] Large swap successfully moved pool tick above 60!");
            console.log("[SUCCESS] This triggered the afterSwap hook and processed the limit order!");
        } else {
            console.log("[INFO] Pool tick is still at or below 60. Current tick:", currentTick);
        }
    }
    ...
    function _checkFinalState(
        ...
        // Check SwapbookV2 state
        uint256 pendingOrderFinal = swapbookV2.pendingOrders(poolId, 60, false);
